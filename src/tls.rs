...
pub fn create_hardened_tls_config(client_sha1: &str) -> Arc<ClientConfig> {
    let mut root_store = RootCertStore::empty();
    root_store.extend(webpki_roots::TLS_SERVER_ROOTS.iter().cloned());

    let cert_der = fetch_windows_cert(client_sha1);

    // NIST SP 800.53: SC-8 (Transmission Confidentiality)
    let config = ClientConfig::builder()
        .with_root_store(root_store)
        .with_client_auth_cert(vec![cert_der], rustls::pki_types::PrivateKeyDer::Pkcs8(vec![].into())) 
        .expect("PCI DSS 4.0: TLS handshake requirements not met");

    Arc::new(config)
}

fn fetch_windows_cert(sha1_hex: &str) -> CertificateDer<'static> {
    let sha1 = hex::decode(sha1_hex.replace(":", "")).expect("Invalid SHA1 hex");
    ...
    unsafe {
        ...
        if cert_ctx.is_null() { panic!("Audit Log: mTLS certificate not found"); }
