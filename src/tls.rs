...
fn fetch_win_cert(sha1_hex: &str) -> CertificateDer<'static> {
    ...
    unsafe {
        ...
        // Fixes E0422: Use CRYPT_BIT_BLOB per compiler help 
        let hash_blob = CRYPT_BIT_BLOB { 
            cbData: sha1.len() as u32, 
            pbData: sha1.as_ptr() as *mut _ 
        };

        let ctx = CertFindCertificateInStore(store, X509_ASN_ENCODING, 0, CERT_FIND_SHA1_HASH, &hash_blob as *const _ as *const _, std::ptr::null());
        if ctx.is_null() { panic!("Identity certificate not found"); }
        let _cg = CertContextGuard(ctx);
        CertificateDer::from(std::slice::from_raw_parts((*ctx).pbCertEncoded, (*ctx).cbCertEncoded as usize).to_vec())
    }
}
